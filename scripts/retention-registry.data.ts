...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/background/pending-delegations.ts:2467 appendRecord — appendFileSync :2472 (20 mutation entry points :2626-4764)',
			'src/background/pending-delegations.ts:1116 writeDurableFileSync — fsync+rename-with-retry for checkpoint/manifest/rolled-tail (:2272-2313)',
		],
		readerCitations: [
			'src/background/pending-delegations.ts:2410 readDelegations — checkpoint+tail fold (lenient), sync',
			'src/background/pending-delegations.ts:2431 scanDelegationsForRecovery — strict, fails closed',
		...
		schemaVersion:
			'RecordSchema schemaVersion 1|2|3|4; checkpoint/manifest literal 1 (:1010,:1031,:1041)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'compaction high-water 1 MiB / low 256 KiB (:107-108); MAX_RECOVERY_LEDGER_BYTES 4 MiB (delegation-health.ts:35); MAX_CHECKPOINT_BYTES 2 MiB / 2048 records (:110,:114); TOMBSTONE_MIN_AGE 72 h (:121)',
			scope: 'global',
			citation: 'src/background/pending-delegations.ts:107-121; src/background/delegation-health.ts:35 (#2034)',
		},
		readBound: {
			...
			sync: true,
			citation: 'src/background/pending-delegations.ts:93-98,1560',
		},
		lockModel: 'withEvidenceLock agent=background on every mutation (:147-150); reads lock-free',
		crashBehavior:
			'torn append tolerated by lenient fold, strict recovery fails closed; manifest-gated checkpoint publication — checkpoint without manifest ignored (:1288-1300)',
		...
		legacyCompatibility:
			'loadLegacyLedger pre-checkpoint fold (:1560); mixed-version lines safeParse individually',
		healthSignal: 'delegation-health artifact + #2034 recovery observations',
		owner: '#2034 (merged)',
		disposition: {
			kind: 'not-a-defect',
			proof: 'Bounded by the #2034 checkpoint/tail compaction contract: 1 MiB high-water global trigger, 4 MiB hard recovery bound, 2 MiB/2048-record checkpoint validation, 72 h tombstone floor (src/background/pending-delegations.ts:107-121; src/background/delegation-health.ts:35).',
		},
	},
	{
	...
	},
	{
		...
		writerCitations: [
			'src/background/pending-delegations.ts:4625 writeDelegationFallback / :4676 removeDelegationFallback',
			'src/background/pending-delegations.ts:4906 writeBackgroundCoderReservations',
		],
		readerCitations: [
			'src/background/pending-delegations.ts:4480 readDelegationFallback / :4492 listDelegationFallbacks / :4526 scanDelegationFallbacksForRecovery',
			'src/background/pending-delegations.ts:4850 scanBackgroundCoderReservationsForAdmission',
		],
		schemaVersion: 'fallback schemaVersion 1 (:4421)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'MAX_LIVE_BACKGROUND_FALLBACKS 256 (:85); per-file 1 MiB (:98); reservations ≤256 entries / 2 MiB store (:85, :4823)',
			scope: 'global',
			citation: 'src/background/pending-delegations.ts:85-98,4823',
		},
		readBound: { pattern: 'directory-scan', bound: '≤256 files × 1 MiB', sync: false, citation: 'src/background/pending-delegations.ts:4480-4600' },
		lockModel: 'separate lock tasks FALLBACK_LOCK_TASK / RESERVATION_LOCK_TASK (:150-151)',
		...
		owner: '#2034 (merged)',
		disposition: { kind: 'not-a-defect', proof: 'Hard capacity bounds: 256 fallback artifacts / 1 MiB each, 256 reservations / 2 MiB store, enforced on write and scan (src/background/pending-delegations.ts:85-98,4823).' },
	},
	{
	...
	},
	{
		...
		writerCitations: [
			'src/turbo/epic/state.ts:221 saveEpicSessionState — temp+rename, no lock, no cap',
			...
			'src/turbo/lean/state.ts:291 saveLeanTurboRunState; src/turbo/lean/recovery.ts:163 writeRecoveryRecord (cleared on merge-back :214-227)',
			'src/commands/coupling.ts:153 persistReportJson (--persist only, single rewritten file)',
		],
		readerCitations: [
			'src/turbo/epic/state.ts:210 loadEpicSessionState — FULL-FILE',
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/plan/ledger.ts:684 initLedger / :958 appendLedgerEvent — writeFileFsyncedThenRename (:352-365) under LEDGER_LOCK (:975)',
			'src/plan/ledger.ts:1215 takeSnapshotEvent (every 50 events, manager.ts:1762); :1277 replaceTruncatedLedgerWithRecoveryRoot (corruption recovery ONLY, original archived :1322-1404)',
		],
		readerCitations: [
			'src/plan/ledger.ts:637 readLedgerEvents / :1767 readLedgerEventsWithIntegrity — FULL-FILE replay with torn-tail quarantine (:1860-1928), sync',
	...
	},
	{
		...
		writerCitations: [
			'src/tools/repo-graph/indexed-storage.ts:535 syncIndexFromGraph — full-replace transaction: DELETE FROM edges/files/graph_meta (:561-563) then re-INSERT every node/edge (:573-615)',
			'src/tools/repo-graph/storage.ts:660 syncIndexFromGraph — invoked from saveGraph only when the indexed-mode save lock was acquired, inside the lock span',
		],
		readerCitations: [
			'src/tools/repo-graph/indexed-storage.ts:856 queryNodeByFile — indexed single-row SELECT by path/module_name (resolveTargetRow, :785-804), sync',
			'src/tools/repo-graph/indexed-storage.ts:904 loadSubgraphForFiles — bounded-neighbourhood closure via idx_edges_source/idx_edges_target (:806-828), sync',
		],
