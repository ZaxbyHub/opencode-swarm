...
/**
 * FB-010.1: poll-until-condition-or-deadline helper for the fire-and-forget
 * skill-invalidation microtask (src/hooks/skill-invalidator.ts), reused from
 * the FB-008 pattern in tests/unit/hooks/skill-invalidator.test.ts and
...
describe('knowledge_remove tool verification tests', () => {
	...
	// ========== Test 9: skill-invalidation wiring through the real tool
	// execute() path (FB-010.1, issue #1717 G11) ==========
	//
	// Prior coverage only exercised writeArchiveTombstoneAndInvalidateSkills'
	// skipTombstone behavior directly (tests/unit/hooks/skill-invalidator.test.ts)
	// or via the curator path (tests/unit/hooks/curator.test.ts) — never through
	// knowledge_remove's actual execute(). This closes that gap: it calls the
	// real tool and asserts BOTH sides of the historical knowledge_remove
	// contract survive the wiring — (a) no tombstone (skipTombstone:true,
	// hard-delete has always been silent) and (b) the fire-and-forget retire
	// invalidation still fires for a linked generated skill.
	describe('Skill invalidation wiring through execute() (FB-010.1)', () => {
		it('writes no archived tombstone but still retires a linked generated skill', async () => {
			const addResult = await knowledge_add.execute(
				{
					lesson: 'Entry linked to a generated skill, later hard-deleted',
					category: 'process',
					tags: ['test'],
					applies_to_agents: ['coder'],
					required_actions: [
						'remove this entry and confirm skill invalidation fires',
					],
				},
				tmpDir,
			);
			...
			// Create a generated skill whose ONLY source is the entry above, so
			// removing it triggers a full retire (not a partial stale).
			...
			// (a) knowledge_remove has always hard-deleted with NO audit
			// tombstone (skipTombstone:true) — that must hold through the real
			// execute() path, not just the shared helper's unit test.
			const events = await readKnowledgeEvents(tmpDir);
			const tombs = events.filter(
				(e: { type?: string }) => e.type === 'archived',
			);
			expect(tombs).toHaveLength(0);

			// (b) the linked skill's retire invalidation still fires via the
			// fire-and-forget microtask, even with the tombstone skipped.
