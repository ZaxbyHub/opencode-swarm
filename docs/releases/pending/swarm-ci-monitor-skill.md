...
- Added an OpenCode-native `swarm-ci-monitor` skill that drives an
  already-reviewed, approved PR to a merged state: monitor CI, exhaustively
  research every failure, fix end-to-end, iterate until all required checks
  are green (max 5 fix cycles), then merge via squash.
...
merge. Each prior skill explicitly fences merge behind the human
("Publication by merge is a user-controlled gate"). `swarm-ci-monitor` is the
first skill in the repo that performs a merge, so it carries extra safety
gates: a 5-iteration cap (mirrors `max_transient_retries`), a concurrency
guard with `--force-with-lease` only, a pre-merge staleness re-check against
the current head SHA, and a post-merge confirmation that verifies the squash
commit via the **local git object DB** rather than the GitHub API (so it does
not share the check-status API's stale-fetch failure mode).
...
- **Flaky-test quarantine is file-level only.** The repo's quarantine file
  ...
  per line. The skill will not jam a test name into it; intra-file flakes are
  fixed at the root or `test.skip()`-ed and escalated.
