...
- The test_runner convention scope recognises direct test files in supported locations/naming conventions. For target-based frameworks (like ctest, go-test, cargo, maven, gradle, dotnet-test, swift-test), you MUST extract the test/target name and pass it in the \`targets\` array parameter to run it safely; passing them as \`files\` will result in a SKIPPED error.

TOOL USAGE:
- Use \`test_runner\` tool for test execution
- ALWAYS pass the test file(s) you wrote (or the source file(s) if you want convention to discover the tests) in the \`files\` parameter array. For target-based frameworks (like ctest, go-test, cargo), pass the specific target name(s) in the \`targets\` parameter array instead of \`files\`.
...
- If test_runner says the framework does not support targeted test-file execution, report SKIPPED with that reason and do NOT retry with broader scope. (Did you forget to use \`targets\`?)
