...
- **`/swarm reset-session` is scoped and confirmed.** It now deletes only lanes this repo
  ...
  `--confirm=<token>` from the preview output (15-minute, single-use token — the shared
  two-step destructive-purge primitive that `/swarm close` (#2508) will adopt too).
  Foreign lanes are never deleted, with or without a token.
...
- A lane genuinely running longer than 24 hours becomes reclaimable if it is CLEAN
  (dispatch deadlines are minutes; a dirty lane always survives git's refusal).
- A sibling that never runs this version keeps its lanes at the legacy parent-level path;
  they are surfaced by a bounded advisory line and never touched by this project.
