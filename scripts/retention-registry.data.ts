...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: [
			'src/observability/otlp-exporter.ts:356 appendSpoolRecord — byte-capped rewrite with drop-oldest',
			'src/observability/otlp-exporter.ts:320 persistState — throttled atomic state write',
		],
		readerCitations: [
			'src/observability/otlp-exporter.ts:330 readSpoolLines — flush-bounded read, sync',
			'src/observability/otlp-exporter.ts:733 readOtlpExporterHealth — health/report read, sync',
		],
		schemaVersion: 'spool record v1 / state v1 (SPOOL_RECORD_VERSION/STATE_VERSION; mapping versions pinned separately #2029)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: { bound: 'spoolMaxBytes (default 1 MiB, min 16 KiB) drop-oldest + spoolMaxAgeMs (default 24h) age sweep with terminal drop reasons (schema.ts observability.export); MAX_SPOOL_LINES_PER_FLUSH 2048; MAX_FLUSH_ITERATIONS 100', scope: 'global', citation: 'src/observability/otlp-exporter.ts:356-389' },
		readBound: { pattern: 'line-bounded', bound: 'flush reads at most MAX_SPOOL_LINES_PER_FLUSH lines + health stat', sync: true, citation: 'src/observability/otlp-exporter.ts:549-552' },
		...
		owner: '#2485',
		disposition: { kind: 'retain-by-design', citation: 'Opt-in bounded export spool: byte+age budgets with drop-oldest terminal reasons (src/observability/otlp-exporter.ts:356-389), privacy-filtered BEFORE append (no raw payload ever spooled, :679-684), restart replay by design (#2485).' },
	},
	{
