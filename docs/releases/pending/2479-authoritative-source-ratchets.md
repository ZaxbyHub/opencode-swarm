...
- Correct the token-formula checker’s imported-module guard so coverage tests do not execute its CLI entry point (and call `process.exit`) during import, preserving Bun’s per-file LCOV artifact and keeping the fail-closed coverage ratchet meaningful.
