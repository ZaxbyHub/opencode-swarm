...
| `diff_context` | `files` OR `diff` (≤ 50 000 chars; only `\r\n\t` whitespace allowed), `top_n` (25) | hunk→symbol mapping with `changedLines` (or file-granularity with a note), per-file impact union with risk; caps: 50 files, 200 hunks, 50 changed lines/symbol; unsafe diff paths are skipped with a warning |
