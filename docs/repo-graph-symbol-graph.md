...
- `source_mode`:
  ...
  - `body` — full range text for every span with an export range;
...
- Each snippet carries `hash` (sha256 hex of the returned text — a content
  ...
  truncated) and `confidence` (resolution-quality score: `1.0` exact target,
  `0.8` resolved neighbor; real edge confidence arrives with KG-11/#1532).
...
- `coverage` (present on every result): `reachedSymbols` / `returnedSymbols` /
  ...
  fallback, so coverage and spans agree).
