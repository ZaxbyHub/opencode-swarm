...
- Makes ready-publication transactional (a generation only becomes `ready` when every observed file has exactly one exact fingerprint) and adds bounded inline retry (3 attempts / 10 s; the deadline may skip a middle attempt but never the final funded one, and denial text reports the true attempt count) for typed transient classes (HEAD timeout/race, mutation during streaming, capture deadline). Exhausted retries and permanent capture failures retain the generation and route to typed recovery (`REVIEWER_CAPTURE_RETRY_EXHAUSTED`, `REVIEWER_CAPTURE_FAILED:<code>`); an incomplete fingerprint set at coder completion is surfaced separately as `REVIEWER_CAPTURE_INCOMPLETE`; only a genuine byte change remains `REVIEWER_SCOPE_STALE` and discards.
...
- Durable receipts recorded under the v1 description (`reviewer-task-files-v1`) can never satisfy a v2 rebuild (every scope this build constructs is v2; the content-hash comparison fails closed); they are never reinterpreted as v2 passes and no proof is backfilled.
## Review-hardening addendum (post-review fixes)

- Merge-back settlement repoints the generation's capture root to the verified primary checkout (the lane directory is torn down immediately after), and lane-rooted background generations route through `mergeback_pending` instead of publishing `ready` unverified.
- Capture binds the pre-open path observation to the opened descriptor (bigint identity snapshot) and re-resolves the path after streaming; a transient parent-ENOENT no longer attests deletion (stable double observation required).
- The reviewer manifest block JSON-quotes file paths (no tag/delimiter injection from crafted filenames) and states the completeness contract explicitly.
- Every typed denial now carries the responsible agent and retryability; per-file capture-failure advisories are sanitized, length-capped, and deduplicated per task.
- A binding/task mismatch aborts any provisioned worktree lane before the typed denial, so the post-fail-closed-region throw cannot leak the lane/branch.
