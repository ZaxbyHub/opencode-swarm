...
| collection_observed | wired | `collectOnce` (`src/tools/dispatch-lanes.ts`) |
...
| provider_terminal_observed | retired | Evidence classification lives in `classifyPrReviewCircuitSignal` (`src/pr-review/circuit.ts`), which filters the same four invalid evidence sources to `ignored` before admission — outcome-equivalent (invalid evidence never becomes a circuit signal). Rule coverage: circuit classifier suites |
