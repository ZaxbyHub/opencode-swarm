...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: [
			'src/dashboard/index.ts:76 writeStatusFile — plain writeFileSync (start/conflict/close rewrite points)',
		],
		readerCitations: ['src/commands/dashboard.ts:25 readStatusFileRecord — single file, async, fail-open null'],
		schemaVersion: 'none',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: { bound: 'single rewritten 4-field snapshot (status/port/url/startedAt; the capability token is never persisted)', scope: 'global', citation: 'src/dashboard/index.ts:76-101' },
