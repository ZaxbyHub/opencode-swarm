...
| `oversight.total_timeout_ms` | number | `120000` | One total wall-clock deadline (10 000–600 000 ms) around a single oversight operation — session creation, prompt, retries, backoff, parse, and cleanup. On timeout the escalated action stays denied, the run pauses, and the reason names the recovery controls (`/swarm full-auto status`, `/swarm full-auto on`, `/swarm handoff`, `/swarm full-auto off`). |
...
**Delegation risk matrix (issue #2103):** a delegation may take the lower-risk supervised/assisted route only when the target's REGISTERED capability map is strictly analysis-only (every tool is a safe analysis tool — e.g. council and curator roles). Delegations to mutation-capable, network, execution, privileged, publication, or unknown roles always escalate to the critic at high risk; a role label or "read-only" prose in the delegation arguments never lowers risk. `strict` mode always escalates every delegation, regardless of `permission_policy.enabled`. Containment, destructive-command, and protected-state gates run independently after any lower-risk decision.

**Structured failure taxonomy and action-local circuits (issue #2103):** tool failures classify through `src/utils/invocation-failure.ts` (provider/shell/filesystem/git/policy/agent/abort families with retry classes — `retry_same`, `retry_fallback`, `repair_then_retry`, `operator_action`, `do_not_retry`). Provider error patterns are evaluated only on model-dispatch error signals, never on tool stdout. A non-transient hard stop blocks the failing action's scope, not the whole invocation; read/diagnose/rescope/handoff/abort tools stay reachable, and a corrected success clears the matching non-immediate circuit.

**Model fallback overrides (issue #2103):** on a provider retry/fallback-eligible error the fallback override is recorded per `(session, swarm, role)` in a bounded store and reaches the next dispatch for that role through its per-call SDK `model` override. A successful response resets the role to its primary model; exhaustion is explicit. The plugin cannot override the model of ordinary OpenCode Task delegations (the host resolves them from the static agent definition); all plugin-dispatched paths (lean lanes, reviewers, critics, oversight, intercept, auto-review, curator, skill-improver) honor the override.
