...
| `pr-review-run-artifacts` | .swarm/pr-review/{run_id}/{findings.jsonl, feedback-handoff.json, trigger-eval.json} + .swarm/pr-review/route-receipts/{session}--{task}.json | governed-content | per-run: findings ≤1000 records/call + 10 MiB read guard; route receipts ≤64 KiB/file; pr-review entries 30 d sweep (per-key; keyspace finite by the 30 d reaper) | line-bounded: 10 MiB findings read guard; route receipts 64 KiB hard read bound | untouched by close — the 30 d sweep owns the pr-review entry reap | not a defect — #2483 |
