...
const tmpDir = mkdtempSync(join(tmpdir(), 'issue-adversarial-test-'));
...
describe('Adversarial Security Tests for issue.ts', () => {
	...
	describe('1. Injection in URL components', () => {
		...
		it('1g. Parse errors strip control characters from the echoed input', () => {
			const result = handleIssueCommand(tmpDir, ['owner/repo\tbad#42']);
			expect(result).toContain('Error:');
		...
		it('1h. Parse errors truncate long echoed input previews', () => {
			...
			const result = handleIssueCommand(tmpDir, [rawInput]);
			expect(result).toContain('Error:');
	...
	describe('3. URL scheme confusion', () => {
		it('3a. javascript: scheme — non-HTTP scheme should fail', () => {
			const result = handleIssueCommand(tmpDir, ['javascript:alert(1)']);
			expect(result).toContain('Error:');
		});

		it('3b. file: scheme — local file access should fail', () => {
			const result = handleIssueCommand(tmpDir, ['file:///etc/passwd']);
			expect(result).toContain('Error:');
		});
	...
	describe('6. Flag injection', () => {
		...
		it('6d. Flag-like string in shorthand format', () => {
			const result = handleIssueCommand(tmpDir, ['owner/--plan#42']);
			expect(result).toContain(
				'issue="https://github.com/owner/--plan/issues/42"',
			);
		});
	...
	describe('9. State isolation between calls', () => {
		it('9a. Alternation between valid and invalid inputs', () => {
			const valid1 = handleIssueCommand(tmpDir, [
			...
			const invalid = handleIssueCommand(tmpDir, ['javascript:alert(1)']);
			expect(invalid).toContain('Error:');

			const valid2 = handleIssueCommand(tmpDir, [
		...
		it('9b. Multiple MODE injections stripped in each call', () => {
			...
			for (const input of inputs) {
				const result = handleIssueCommand(tmpDir, [input]);
				expect(result).not.toContain('EXECUTE');
				expect(result).not.toContain('pr_review');
				expect(result).toContain(
