...
### Category 2 — Background delegation, PR monitor/feedback, lane sidecars (14 rows)
...
| Row id | Path grammar | State class | Write limit (scope) | Read bound | Close policy | Disposition → owner |
|---|---|---|---|---|---|---|
| `plan-ledger` | .swarm/swarm.db plan_ledger_* + portable plan-ledger.jsonl and recovery archives | authoritative | exact append-only history; no sampling or truncation (global) | JSONL full-file in file-shadow; ordered SQLite rows after cutover | database and portable export archived, then terminal state removed | retain by design — this-gate; dual-store lifecycle and direct-file shadow/export reviewed in #2484 |
