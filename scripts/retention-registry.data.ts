...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: [
			'src/hooks/delegation-gate/dead-lane-reclaim.ts:131 recordDeadLaneReclaim — dedupe-by-lanePath merge, then writeStore (:113) rewrites the bounded store atomically via atomicWriteSwarmFileSync (:123); the only writer — the three strand sites (worktree-isolation handleStrandedLane, delegation-gate terminal-failure path, reset-session EBUSY bulk-clean path) all call this one seam (issue #2599)',
			'src/hooks/delegation-gate/dead-lane-reclaim.ts:222 reclaimDeadLanes — the consuming pass at next start; fs.rmSync force removes the store file once no entries remain (:301)',
		],
		readerCitations: [
			'src/hooks/delegation-gate/dead-lane-reclaim.ts:90 readStore — single file, sync readFileSync bounded by MAX_DEAD_LANE_RECLAIM_BYTES (:92-94); missing/oversized/malformed reads fail to the empty store, never throw',
		...
		schemaVersion: '1 (DeadLaneReclaimStore.schemaVersion, dead-lane-reclaim.ts:53)',
		stateClass: 'operational',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'MAX_DEAD_LANE_RECLAIMS=512 entries + MAX_DEAD_LANE_RECLAIM_BYTES=256 KiB serialized, drop-oldest enforced on BOTH axes in writeStore (dead-lane-reclaim.ts:36-37,113-128)',
			scope: 'global',
			citation: 'src/hooks/delegation-gate/dead-lane-reclaim.ts:36-37,116-128',
		},
		readBound: {
			pattern: 'full-file',
			bound: 'single ≤256 KiB JSON, read once per recordDeadLaneReclaim and once per startup reclaim pass',
			sync: true,
			citation: 'src/hooks/delegation-gate/dead-lane-reclaim.ts:90-109',
		},
		...
		healthSignal: 'WORKTREE_LANE_STRANDED advisory (delegation-gate / worktree-isolation / reset-session) + "Preserved dead lane …" startup warnings (init-orphan-recovery); both surface the strand reason without raw output text',
		owner: '#2599',
		disposition: {
			kind: 'not-a-defect',
			proof: 'Hard dual bound 512 entries / 256 KiB serialized with drop-oldest on both axes (src/hooks/delegation-gate/dead-lane-reclaim.ts:36-37,116-128); the store is a transient pending-reclaim queue — entries are consumed at the next start and the file deleted when empty (:222,:301), with fail-closed ownership/dirty gates preventing destructive replay (issue #2599 acceptance).',
		},
	},
	{
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/plan/ledger.ts:684 initLedger / :958 appendLedgerEvent — writeFileFsyncedThenRename (:352-365) under LEDGER_LOCK (:966)',
			'src/plan/ledger.ts:1215 takeSnapshotEvent (every 50 events, manager.ts:1762); :1268 replaceTruncatedLedgerWithRecoveryRoot (corruption recovery ONLY, original archived :1313-1395)',
		],
		readerCitations: [
			'src/plan/ledger.ts:637 readLedgerEvents / :1767 readLedgerEventsWithIntegrity — FULL-FILE replay with torn-tail quarantine (:1849-1917), sync',
			'src/plan/ledger.ts:595 getLatestLedgerSeq / :1971 loadLastApprovedPlan — full scans',
		],
