...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		closePolicy: 'archived-only — ARCHIVE_ARTIFACTS (close.ts:421-423); deliberately NOT cleaned (cross-session store; compaction is the bounded-retention mechanism, close.ts:416-424 docblock)',
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		crashBehavior: 'atomic single-file rewrites (PID-scoped tmp + byte-verified rename; in-memory manifest/framing validation pre-rename); torn trailing line skipped + counted corrupt, re-framed on next append; legacy header-less files migrate in bounded fold passes',
		closePolicy: 'finalizeShellAuditForClose under the store lock (legacy drain to convergence + compaction + validated cut) BEFORE the plain session/ directory archive copy, then archived (via the session dir copy) and cleaned (session dir lifecycle); the lock file is released (unlinked) by finalize so a stale lock is never archived',
		...
		legacyCompatibility: 'header-less files read bounded (newest window); legacy records re-redact at render time through the CURRENT policy (redactShellCommand/redactPath in guardrail-log-service) — no legacy bypass; the fold pass preserves lines byte-for-byte',
