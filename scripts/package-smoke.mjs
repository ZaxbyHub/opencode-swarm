	...
	try {
		...
		writeFileSync(
			...
			[
				...
				"const nativePlan = path.join(projectDir, '.opencode', 'skills', 'swarm-plan', 'SKILL.md');",
				"const privatePlan = path.join(projectDir, '.swarm', 'bundled-skills', 'swarm-plan', 'SKILL.md');",
		...
		writeFileSync(
			...
			[
				...
				"const result = await evaluationV1.evaluateCandidate({ projectRoot: root, tasks: [task], baseline, candidate, split: 'validation', seed: 'package-smoke', models: ['configured'], budgets: { maxTasks: 1, maxRepetitions: 1, maxConcurrency: 2, maxTaskTimeMs: 10_000, maxRetries: 0, maxOutputBytes: 1024 }, executor: async () => ({ status: 'completed', text: '{\\\"v\\\":1,\\\"caught\\\":true}', durationMs: 1, cost: { source: 'reported', usd: 0 } }) });",
				"const replay = await evaluationV1.evaluateCandidate({ projectRoot: root, tasks: [task], baseline, candidate, split: 'validation', seed: 'package-smoke', models: ['configured'], budgets: { maxTasks: 1, maxRepetitions: 1, maxConcurrency: 2, maxTaskTimeMs: 10_000, maxRetries: 0, maxOutputBytes: 1024 }, executor: async () => ({ status: 'completed', text: '{\\\"v\\\":1,\\\"caught\\\":true}', durationMs: 1, cost: { source: 'reported', usd: 0 } }) });",
				"if (result.run.status !== 'complete') throw new Error(`package evaluation run did not complete: ${JSON.stringify(result.run)}`);",
