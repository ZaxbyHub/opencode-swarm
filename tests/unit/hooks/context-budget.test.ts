...
describe('Issue #2068: real ToolPart shape', () => {
	...
	test('T12: mixed message masks only non-exempt tool parts', async () => {
		// Tune so masking the one non-exempt output reaches target (no pruning,
		// which would replace the whole message including the exempt part).
		const config = baseConfig({
		...
		const parts = output.messages[0].parts;
		// Exempt tool part retained intact.
		...
		// Non-exempt tool part replaced with a masked text placeholder.
	...
	test('T11: text part + tool part — masking replaces only the tool part; both counted', async () => {
		// Tune so masking the tool output alone brings usage under target, so the
		// message is NOT subsequently pruned (which would replace the text part).
		const config = baseConfig({
		...
		// The architect warning on the last user message proves it.
		const userText = output.messages[1].parts[0].text as string;
		expect(userText).toContain('CONTEXT CRITICAL');
	});
	...
	test('T13: pruning preserves exempt tool output even when message is pruned', async () => {
		// Force pruning: large user message drives usage far over target so the
		// old tool message is removed via applyObservationMasking.
		const config = baseConfig({
		...
		const messages = [
			toolMsg('retrieve_summary', exemptOutput), // exempt — must survive pruning
			{
				info: { role: 'user', agent: 'architect' },
		...
		// The exempt tool part must retain its original output (not pruned).
