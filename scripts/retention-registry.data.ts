...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: [
			'src/hooks/skill-usage-log.ts:553 appendSkillUsageEntry — appendFileSync :642-646; enqueues into the sidecar FIRST (:633-640) before the JSONL append',
			'src/hooks/skill-usage-log.ts:1574 pruneSkillUsageLog — atomic temp+rename rewrite :1672-1677, applies applyRetention (:1032) and the marker-drop rewrite',
			'src/hooks/skill-usage-log.ts:344 appendFeedbackAppliedMarker — appendFileSync :359 (legacy pre-migration acknowledgment writer, retained for round-trip)',
		],
		readerCitations: [
			'src/hooks/skill-usage-log.ts:788 readSkillUsageEntries — bounded via readLogSlice :669-709, readMaxBytes=1,677,722 B, sync',
			'src/hooks/skill-usage-log.ts:763 readSkillUsageEntriesWithCoverage — same bound, additionally reports SkillUsageReadCoverage (truncatedRead + sidecar manifest coverage), sync',
			'src/hooks/skill-usage-log.ts:316 readFeedbackAppliedEntryIds — FULL-FILE :320 (legacy pre-migration reader only), sync',
			'src/hooks/skill-usage-log.ts:1980 applySkillUsageFeedback — reads the AUTHORITATIVE sidecar queue only (never the JSONL), async',
			'src/hooks/skill-usage-log.ts:826 readSkillUsageEntriesTail — TAIL 64 KiB (TAIL_BYTES_DEFAULT :800), sync',
		...
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'HARD GLOBAL ceiling across all skills and marker types (issue #2038): maxEntries=5,000 / maxBytes=1.5 MiB / maxAgeMs=90d, with a per-skill floorPerSkill=20 guaranteed retained window; enforced by applyRetention (skill-usage-log.ts:1032-1121) on every compaction pass',
			scope: 'global',
		...
		readBound: {
			...
			sync: true,
			citation: 'src/hooks/skill-usage-log.ts:669-709 readLogSlice; :800 TAIL_BYTES_DEFAULT',
		},
		...
		crashBehavior: 'malformed JSONL lines skipped by JSON.parse try/catch (parseEntriesFromText :711-724); an overlong unassemblable line in the streaming reader is dropped and counted, not buffered without bound (streamLogLines :377-411); prune/compaction rewrite is atomic temp+rename (pruneSkillUsageLog :1672-1677); sidecar save is atomic temp+rename (savePendingDocument :787 -> savePendingDocumentAt, skill-usage-pending.ts:795-845, writeFileSync :812 + renameSync :813)',
		closePolicy: 'untouched — persists across sessions',
		resetPolicy: 'not reset',
		legacyCompatibility: 'normalizeComplianceVerdict maps legacy violation→violated (skill-usage-log.ts:146-148); legacySkillUsageId mints a deterministic content-hash id for pre-id entries (:264-275); one-time migrateLegacyLog (:1289-1420) folds legacy actionable entries minus any feedback_applied-acknowledged ids into the sidecar; the whole migration runs on a staged copy that is published to the in-memory document only after savePendingDocument returns (stagePendingDocument :1207-1219 / adoptStagedDocument :1222-1231), so a failed sidecar write leaves the JSONL marker lines intact instead of dropping them ahead of a queue that was never written',
		...
		disposition: {
			kind: 'retain-by-design',
			citation:
				'Issue #2038 landed a hard global byte/age/count bound (SKILL_USAGE_LIMITS: maxEntries=5,000/maxBytes=1.5MiB/maxAgeMs=90d/floorPerSkill=20, src/hooks/skill-usage-pending.ts:85-101) enforced by applyRetention (src/hooks/skill-usage-log.ts:1032-1121), a bounded deterministic full reader (readLogSlice, :669-709, readMaxBytes=1,677,722 B) that reports truncatedRead rather than silently degrading, a shared stale-breakable lock (.swarm/skill-usage.lock, skill-usage-pending.ts:493-503), atomic temp+rename writes on both the JSONL compaction path (:1672-1677) and the new authoritative sidecar (skill-usage-pending.ts:795-845), and the skill_usage_health counts-only observability signal (skill-usage-pending.ts:1334-1385). The correctness gap the bound creates — an evicted-from-JSONL actionable verdict losing its feedback signal — is closed by enqueueSkillUsageFeedback (skill-usage-pending.ts:1159-1184), called BEFORE the JSONL append (skill-usage-log.ts:633-640) so eviction from the operational stream can never lose an un-consumed compliant/violated verdict; see the new skill-usage-pending row for that authoritative store\'s own bound. Enforcement is covered by tests/unit/hooks/skill-usage-bounds.test.ts (global ceiling, retention order, corrupt-line durability, migration) and tests/unit/hooks/skill-usage-pending.test.ts (queue budgets, terminal outcomes, quarantine), alongside tests/unit/hooks/skill-usage-log.test.ts and skill-usage-feedback.test.ts.',
		},
	},
	{
		...
		writerCitations: [
			'src/hooks/skill-usage-pending.ts:787 savePendingDocument — delegates to savePendingDocumentAt :795, writeFileSync tmp :812, renameSync :813 (atomic replace)',
			'src/hooks/skill-usage-pending.ts:1159 enqueueSkillUsageFeedback — under the lock, merges + bounds + saves (:1177-1180); the ONLY path exempt from skip-not-force (throws on lock failure)',
		],
		readerCitations: [
			'src/hooks/skill-usage-pending.ts:720 loadPendingDocument — via loadPendingDocumentAt :731, bounded by readMaxBytes=1,677,722 B (:749); oversized/corrupt documents are quarantined, never silently reset to empty (:773)',
			'src/hooks/skill-usage-pending.ts:855 readPendingManifest — cheap manifest-only read, statSync-mtime-keyed cache (:870-876), no records array materialized',
			'src/hooks/skill-usage-log.ts:763 readSkillUsageEntriesWithCoverage — folds readPendingManifest coverage into the JSONL read window',
			'src/hooks/skill-usage-log.ts:1980 applySkillUsageFeedback — sole consumer of the records array (via claimFeedbackRecords :1854-1891), computes compliant/violated deltas from queue records only, never from JSONL entries',
		],
		...
		readBound: {
			...
			sync: true,
			citation: 'src/hooks/skill-usage-pending.ts:720-781 loadPendingDocument',
		},
		lockModel: 'single shared .swarm/skill-usage.lock (openSync wx-create, stale-broken after SKILL_USAGE_LOCK_STALE_MS=5min, skill-usage-pending.ts:493-503); enqueue is exempt from skip-not-force (acquireSkillUsageLockOrThrow :526-538, 5 attempts/10ms, throws and aborts the caller\'s append on failure); maintenance/consumption skip on lock failure, never force',
		crashBehavior: 'atomic temp+rename replace (savePendingDocument :787 -> savePendingDocumentAt :795-845, writeFileSync :812 + renameSync :813); a corrupt or oversized document is quarantined (renamed to a timestamped .corrupt- file) rather than silently discarded (quarantinePendingDocument :691-710, invoked from loadPendingDocumentAt :773); an in_flight record whose claim outlives the lock stale-break window is resolved to uncertain — survives, stays visible, never replayed (resolveStaleInFlight :1195-1212)',
		closePolicy: 'untouched — persists across sessions (same lifecycle as .swarm/skill-usage.jsonl, which it backs)',
		resetPolicy: 'not reset',
		legacyCompatibility: 'migrated: false on a fresh/absent document signals the one-time legacy migration has not run; migrateLegacyLog (skill-usage-log.ts:1289-1420) folds pre-existing JSONL actionable entries (minus feedback_applied-acknowledged ids) into this store on first lock-taking touch, then sets migrated: true',
		healthSignal: 'skill_usage_health — same counts-only payload as the skill-usage row (buildSkillUsageHealthPayload, skill-usage-pending.ts:1334-1369), emitted via emitSkillUsageHealth (:1372-1385)',
		owner: '#2038 (implemented)',
		disposition: {
			kind: 'retain-by-design',
			citation:
				'This store is deliberately AUTHORITATIVE, not derived-rebuildable: it is the sole durable record of which actionable (compliant/violated) skill-usage verdicts have not yet been folded into knowledge confidence via bumpKnowledgeConfidenceBatchResult (src/hooks/knowledge-store.ts:1293), driven by applySkillUsageFeedback (src/hooks/skill-usage-log.ts:1980-2083). Nothing else on disk records that fact once an entry is evicted from the bounded JSONL operational stream (issue #2038 requirement: eviction from skill-usage.jsonl must lose no correctness signal). It carries its own hard global bound (queueMaxRecords=5,000/queueMaxBytes=512KiB, src/hooks/skill-usage-pending.ts:85-101) enforced by enforceQueueBounds (:1015-1072, oldest-uncertain-first eviction; an actionable record evicted by the budget is counted as pending_evicted + pressure rather than dropped, a deliberate divergence from approved plan section 4 recorded at evictionRank :972-1009 — every discard counted via the durable counters, never silent), atomic temp+rename writes (savePendingDocument :787 via savePendingDocumentAt :795-845), a shared stale-breakable lock, and the skill_usage_health signal. Enforcement is covered by tests/unit/hooks/skill-usage-pending.test.ts and tests/unit/hooks/skill-usage-bounds.test.ts.',
		},
	},
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/background/pending-delegations.ts:2171 appendRecord — appendFileSync :2176 (16 mutation entry points :2323-3959)',
			'src/background/pending-delegations.ts:921 writeDurableFileSync — fsync+rename-with-retry for checkpoint/manifest/rolled-tail (:1976-2017)',
		],
		readerCitations: [
			'src/background/pending-delegations.ts:2114 readDelegations — checkpoint+tail fold (lenient), sync',
			'src/background/pending-delegations.ts:2135 scanDelegationsForRecovery — strict, fails closed',
		...
		schemaVersion: 'RecordSchema schemaVersion 1|2|3; checkpoint/manifest literal 1 (:815,:836,:846)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		writeLimits: {
		...
		readBound: {
			...
			sync: true,
			citation: 'src/background/pending-delegations.ts:77-82,1344',
		},
		lockModel: 'withEvidenceLock agent=background on every mutation (:129-132); reads lock-free',
		crashBehavior: 'torn append tolerated by lenient fold, strict recovery fails closed; manifest-gated checkpoint publication — checkpoint without manifest ignored (:1085-1097)',
		closePolicy: 'archived-only — ARCHIVE_ARTIFACTS (close.ts:418-420); deliberately NOT cleaned (cross-session store; compaction is the bounded-retention mechanism, close.ts:413-421 docblock)',
		resetPolicy: 'reset/reset-session do not delete',
		legacyCompatibility: 'loadLegacyLedger pre-checkpoint fold (:1344); mixed-version lines safeParse individually',
