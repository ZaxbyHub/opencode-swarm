...
- Treat controller absence as Profile B, not an error: use fresh-context subagents, preserve read-only workflow-lane handling and exact-head provenance, keep `output_ref` recovery intact, preserve the PR publication contract, and report BLOCKED merely because the controller is absent or the only alternative would be a degraded review.
