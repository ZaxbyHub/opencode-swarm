...
| `pr-review-run-artifacts` | .swarm/pr-review/{run_id}/{findings.jsonl, feedback-handoff.json, trigger-eval.json} + .swarm/pr-review/route-receipts/{encoded-session}--{encoded-task}.json (each trimmed identity is UTF-8/base64url encoded with `-` mapped to `~`; components over 180 characters use deterministic `sha256_<hex>`, so segments stay bounded and cannot contain the `--` separator) | governed-content | per-run: findings ≤1000 records/call + 10 MiB read guard; route receipts ≤64 KiB/file; run directories and each route-receipt file age-prune independently at 30 d | line-bounded: 10 MiB findings read guard; route receipts 64 KiB hard read bound | untouched by close — the 30 d sweep owns the run and route-receipt reap | not a defect — #2483 |
| `pr-review-workflow-gate-state` | .swarm/pr-workflow-gates/{session-stem}.json (+ .lock) | authoritative | one bounded state object per workflow session; atomic projection under the gate lock (per-key; keyspace finite by the 30 d reaper) | indexed: one bounded session file | untouched by close — the 30 d sweep owns workflow shadow lifecycle | not a defect — #2385 |
