...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/background/pending-delegations.ts:2231 appendRecord — appendFileSync :2236 (16 mutation entry points :2383-4021)',
			'src/background/pending-delegations.ts:981 writeDurableFileSync — fsync+rename-with-retry for checkpoint/manifest/rolled-tail (:2036-2077)',
		],
		readerCitations: [
			'src/background/pending-delegations.ts:2174 readDelegations — checkpoint+tail fold (lenient), sync',
			'src/background/pending-delegations.ts:2195 scanDelegationsForRecovery — strict, fails closed',
		...
		schemaVersion: 'RecordSchema schemaVersion 1|2|3; checkpoint/manifest literal 1 (:875,:896,:906)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'compaction high-water 1 MiB / low 256 KiB (:94-95); MAX_RECOVERY_LEDGER_BYTES 4 MiB (delegation-health.ts:34); MAX_CHECKPOINT_BYTES 2 MiB / 2048 records (:97,:101); TOMBSTONE_MIN_AGE 72 h (:108)',
			scope: 'global',
			citation: 'src/background/pending-delegations.ts:94-108; src/background/delegation-health.ts:34 (#2034)',
		},
		readBound: {
			...
			sync: true,
			citation: 'src/background/pending-delegations.ts:82-87,1404',
		},
		lockModel: 'withEvidenceLock agent=background on every mutation (:134-137); reads lock-free',
		crashBehavior: 'torn append tolerated by lenient fold, strict recovery fails closed; manifest-gated checkpoint publication — checkpoint without manifest ignored (:1145-1157)',
		...
		legacyCompatibility: 'loadLegacyLedger pre-checkpoint fold (:1404); mixed-version lines safeParse individually',
		healthSignal: 'delegation-health artifact + #2034 recovery observations',
		owner: '#2034 (merged)',
		disposition: {
			kind: 'not-a-defect',
			proof: 'Bounded by the #2034 checkpoint/tail compaction contract: 1 MiB high-water global trigger, 4 MiB hard recovery bound, 2 MiB/2048-record checkpoint validation, 72 h tombstone floor (src/background/pending-delegations.ts:94-108; src/background/delegation-health.ts:34).',
		},
	},
	{
		...
		writerCitations: ['src/background/delegation-health.ts:331 writeDelegationHealthArtifact — temp+rename-retry'],
		readerCitations: ['src/background/delegation-health.ts:308 readDelegationHealthArtifact — single-file, sync; :538 collectDelegationLedgerHealth'],
		...
		readBound: { pattern: 'indexed', bound: 'single small JSON artifact', sync: true, citation: 'src/background/delegation-health.ts:308' },
		...
		owner: '#2034 (merged)',
		disposition: { kind: 'not-a-defect', proof: 'Single rewritten bounded artifact describing already-bounded stores; atomic + fail-open (src/background/delegation-health.ts:331,419).' },
	},
	{
