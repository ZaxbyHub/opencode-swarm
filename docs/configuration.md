...
- **Legacy v1 receipts.** Receipts recorded before the v2 manifest (description `reviewer-task-files-v1`) can never satisfy a v2 rebuild and fail closed with a one-time `REVIEW_SCOPE_LEGACY_V1` re-review signal. No v1 evidence is reinterpreted or backfilled.
