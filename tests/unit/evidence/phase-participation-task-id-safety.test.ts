...
describe('phase participation task-id safety', () => {
	...
	test('oversized plan context accepts a known explicit numeric task', async () => {
		writePlan(
			directory,
			Array.from(
				{ length: TASK_ID_RESOLUTION_LIMITS.maxKnownIds + 1 },
		...
		await observePhaseParticipationToolResult({
			...
			parentSessionId: 'parent-explicit',
			callId: 'call-explicit',
			output: runningOutput('child-explicit'),
		});

		expect(readPendingTaskId(directory)).toBe('1.1025');
	});
});
