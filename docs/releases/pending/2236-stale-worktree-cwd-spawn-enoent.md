...
**This output check is not a security control and should not be read as one:** anything that can name the
binary can also make it print `git version 2.43.0`. What it does catch is an *accidental* non-git target — a
`/bin/true`, a broken `xcode-select` shim, a wrapper that exits 0 without being git. The provenance rule above
is the actual control. A refused or unusable value is still never fatal:
