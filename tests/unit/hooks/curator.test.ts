...
{"type": "event2", "timestamp": "2024-01-15T10:01:00Z"}`;
	...
	describe('applyCuratorKnowledgeUpdates', () => {
		...
		// tombstone, no retire/stale microtask). Mirrors the canonical
		// knowledge-archive.test.ts pattern at lines 435-476.
		it('G11: curator archive writes a tombstone and retires linked skills', async () => {
			const entries: SwarmKnowledgeEntry[] = [
				{
					id: 'G11-src',
					tier: 'swarm',
					lesson: 'G11 source lesson',
					category: 'process',
					tags: [],
					scope: 'global',
					confidence: 0.85,
					...
					created_at: '2026-01-01T00:00:00Z',
					updated_at: '2026-01-01T00:00:00Z',
					project_name: 'g11-test',
				},
			];
			createKnowledgeFile(tempDir, entries);

			// Create a generated skill whose source_knowledge_ids references G11-src.
			...
			// Wait for the invalidator's microtask (SC-004 timing) — poll for
			// the observable side effect instead of a fixed delay (FB-008).
			await waitForFileToExist(path.join(skillDir, 'retired.marker'));

			// G11 tombstone: an 'archived' event with actor:'curator' was written.
			...
			const tombs = events.filter(
				...
				} => e.type === 'archived' && e.actor === 'curator',
			);
			expect(tombs).toHaveLength(1);
			expect(tombs[0].mode).toBe('archive');
			// The real pre-mutation status ('established') is captured.
			expect(tombs[0].previous_status).toBe('established');

			// The linked skill was retired (all sources archived → retire).
			expect(fs.existsSync(path.join(skillDir, 'retired.marker'))).toBe(true);
		});

		it('G11: tombstone captures the real previous_status (not hardcoded)', async () => {
			// Start the entry as 'candidate' to distinguish from a hardcoded
			// 'established' default.
			const entries: SwarmKnowledgeEntry[] = [
				{
					id: 'G11-prev',
					tier: 'swarm',
					lesson: 'prev-status lesson',
					category: 'process',
					tags: [],
					scope: 'global',
					confidence: 0.5,
			...
			const events = await readKnowledgeEvents(tempDir);
			const tomb = events.find(
				(e: {
					type?: string;
				}): e is { type: 'archived'; previous_status?: string } =>
					e.type === 'archived',
			);
			expect(tomb).toBeDefined();
			expect(tomb?.previous_status).toBe('candidate');
		});

		// FB-010.2: the two existing G11 tests above only exercise the "full
		// retire" branch (single-source entry, fully archived → retired.marker).
		// This test exercises the PARTIAL "stale" branch through
		// applyCuratorKnowledgeUpdates: a skill generated from TWO source
		// knowledge entries, only ONE of which is archived via a curator
		// 'archive' recommendation. Fixture modeled on
		// tests/unit/hooks/knowledge-archive.test.ts's "archive partial
		// sources creates stale.marker (not retire)" case.
		it('G11: curator archive of ONE of multiple sources marks the linked skill stale, not retired', async () => {
			const entries: SwarmKnowledgeEntry[] = [
				{
					id: 'G11-multi-a',
					tier: 'swarm',
					lesson: 'G11 multi-source lesson A',
					category: 'process',
					tags: [],
					scope: 'global',
					confidence: 0.85,
				...
				},
				{
					id: 'G11-multi-b',
					tier: 'swarm',
					lesson: 'G11 multi-source lesson B',
					category: 'process',
					tags: [],
					scope: 'global',
					confidence: 0.85,
			...
			// A single generated skill sourced from BOTH entries.
			const skillDir = path.join(
				tempDir,
				'.opencode',
				'skills',
				'generated',
				'g11-multi-source-skill',
			);
			fs.mkdirSync(skillDir, { recursive: true });
			fs.writeFileSync(
				path.join(skillDir, 'SKILL.md'),
			...
			// Only archive ONE of the two sources.
			const recommendations: KnowledgeRecommendation[] = [
				{
					action: 'archive',
					entry_id: 'G11-multi-a',
					lesson: 'G11 multi-source lesson A',
					reason: 'archive only one of two sources',
				},
			];
			...
			// Poll for the stale.marker — the microtask must run before either
			// marker can exist.
			await waitForFileToExist(path.join(skillDir, 'stale.marker'));

			// Not all sources are archived → the skill is marked STALE, not
			// fully retired.
		...
		// FB-010.3: guards the `r.action === 'archive'` filter that gates the
		// G11 invalidation loop (src/hooks/curator.ts). If that filter were
		// ever broadened (e.g. to include 'promote'), a tombstone would be
		// written for a recommendation that never archived anything.
		it('G11: recommendations with no archive action write no archived tombstone', async () => {
			// Two separate entries — one 'promote', one 'flag_contradiction' —
			// so each recommendation applies to its own entry (transactKnowledge
			// matches recommendations to entries by entry_id via `find`, so
			// reusing one entry_id across two recs would only apply the first).
			const entries: SwarmKnowledgeEntry[] = [
				{
					id: 'G11-promote-only',
					tier: 'swarm',
					lesson: 'lesson eligible for promotion',
					category: 'process',
					tags: [],
					scope: 'global',
					confidence: 0.5,
				...
				{
					id: 'G11-contradiction-only',
					tier: 'swarm',
					lesson: 'lesson flagged for contradiction',
					category: 'process',
					tags: [],
					scope: 'global',
					confidence: 0.5,
			...
			const recommendations: KnowledgeRecommendation[] = [
				{
					action: 'promote',
					entry_id: 'G11-promote-only',
					lesson: 'lesson eligible for promotion',
					reason: 'promote only, no archive',
				},
				{
					action: 'flag_contradiction',
					entry_id: 'G11-contradiction-only',
					lesson: 'lesson flagged for contradiction',
					reason: 'contradiction only, no archive',
				},
			];
