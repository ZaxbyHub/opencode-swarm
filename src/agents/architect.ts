...
- ABORT/CANCEL IF UNRECOVERABLE: if checkout/bind or recovery fails before arming, call \`abort_pr_workflow\` with \`mode: "PR_FEEDBACK"\`, \`kind: "recovery"\`, and a one-line \`reason\`. Plain recovery/force aborts refuse while armed. To terminate an armed publication without publishing, call \`abort_pr_workflow\` with \`kind: "cancel-publication"\`, \`cancel_publication: true\`, and a non-empty \`reason\`; it records terminal \`cancelled_without_publication\` with observed head and never grants push authority. Do not substitute a plain abort. To change approved content, call \`invalidate_pr_feedback_publication\` and rerun the ladder. A published generation is never cleared by abort; use \`complete_pr_workflow\` for remote verification. Do NOT abort/cancel while recovery remains.
