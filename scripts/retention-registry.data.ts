...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		...
		writerCitations: [
			'src/turbo/epic/state.ts:524 saveEpicSessionState — per-session SQLite CAS with post-commit JSON projection',
			...
			'src/turbo/lean/state.ts:596 saveLeanTurboRunState — per-session SQLite CAS; src/turbo/lean/recovery.ts:163 writeRecoveryRecord (cleared on merge-back :214-227)',
			'src/commands/coupling.ts:153 persistReportJson (--persist only, single rewritten file)',
		],
		readerCitations: [
			'src/turbo/epic/state.ts:513 loadEpicSessionState — indexed SQLite state row',
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/db/project-db.ts getProjectDb — canonical-identity cache key, WAL + synchronous NORMAL default + busy_timeout 5000 + foreign_keys; versioned migrations v1-v25 with failed-migration recovery (migration_failures + marker fallback); closeProjectDb runs a best-effort TRUNCATE→PASSIVE WAL checkpoint',
			'src/db/coordination-store.ts transitionCoordinationState — FULL-durability event+state CAS, bounded event retention, leases, and one-time imports (#2481)',
		...
		schemaVersion: 'schema_migrations versioned (25)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
