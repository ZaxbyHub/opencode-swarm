...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/background/pending-delegations.ts:2302 appendRecord — appendFileSync :2307 (16 mutation entry points :2454-4093)',
			'src/background/pending-delegations.ts:1052 writeDurableFileSync — fsync+rename-with-retry for checkpoint/manifest/rolled-tail (:2107-2148)',
		],
		readerCitations: [
			'src/background/pending-delegations.ts:2245 readDelegations — checkpoint+tail fold (lenient), sync',
			'src/background/pending-delegations.ts:2266 scanDelegationsForRecovery — strict, fails closed',
		...
		schemaVersion:
			'RecordSchema schemaVersion 1|2|3; checkpoint/manifest literal 1 (:946,:967,:977)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		writeLimits: {
		...
		readBound: {
			...
			sync: true,
			citation: 'src/background/pending-delegations.ts:82-87,1475',
		},
		lockModel: 'withEvidenceLock agent=background on every mutation (:134-137); reads lock-free',
		crashBehavior:
			'torn append tolerated by lenient fold, strict recovery fails closed; manifest-gated checkpoint publication — checkpoint without manifest ignored (:1216-1228)',
		...
		legacyCompatibility:
			'loadLegacyLedger pre-checkpoint fold (:1475); mixed-version lines safeParse individually',
