...
**Plan freeze after approval.** Once a plan receives an APPROVED verdict, post-approval `save_plan` calls are permitted only for **non-material changes** (renumbering, description clarifications, dependency rewiring between existing tasks, typo fixes). Any **material change** — new tasks, removed tasks, requirement text changes, scope reassignment between tasks, or phase structure changes — invalidates the approval and triggers **one** new critic pass. A material change after approval is itself a defect class: future sessions must not learn from a non-existent precedent.

Detection is left to the architect: if the new `phases` text differs from the approved snapshot in any `fr_refs`, `acceptance`, `files_touched`, `description`, `name`, `depends`, or `removed_task_ids` field, treat it as a material change and re-run critic-gate. The material-change surface includes task `description` and phase `name` because `save_plan` derives requirement text from them (see `src/tools/save-plan.ts:1016-1019,1053-1058` and `src/config/plan-schema.ts:74-76`); a requirement can be materially changed in description without touching `fr_refs`, `acceptance`, or `files_touched`, and would otherwise retain a stale critic approval. This is the issue #1994 disposition P1.
