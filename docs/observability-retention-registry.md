...
For issue #2487, `scripts/issue-2487-compatibility.data.ts` is the companion
compatibility contract for every `swarm.db` table and named legacy stream;
`bun run check:issue-2487-compatibility` independently derives the v37
migration/table ground truth and scans production `src/` reachability before
accepting the registry. A reachable legacy source therefore cannot be marked
`retired` by declaration alone.
...
| `pr-review-workflow-gate-state` | .swarm/pr-workflow-gates/{session-stem}.json (+ .lock) | authoritative | one bounded state object per workflow session (per-key; keyspace finite by the retention reaper) | indexed: one bounded session file | untouched by close — retention sweep owns workflow shadow lifecycle | not a defect — #2385; direct-file exemption (#2385) |
...
| Row id | Path grammar | State class | Write limit (scope) | Read bound | Close policy | Disposition → owner |
|---|---|---|---|---|---|---|
| `project-db` | .swarm/swarm.db (+ transient -wal/-shm sidecars; schema v37) | authoritative | indexed config/profile plus #2481 coordination event/state/lease/import rows; payload ≤1 MiB, identifiers ≤512 chars; events retain ≤2048/stream and prune non-head history toward a 100k global target without deleting a stream waterline; idempotency fences retain ≤8192/stream and ≤400k globally; domain stores apply session/terminal/tombstone retention (session/global) | indexed: PK/namespace/status/event-stream/lease-expiry lookups, query LIMIT ≤5000; compatibility gate independently verifies all 18 tables | archived+cleaned — closeProjectDb releases Windows locks before cleanup; archive/restore is qualified by issue-2487 | not a defect — #2030, #2480, #2481, #2487 |
...
| `planned-legacy-retirement` | retention-owned issue #2487 legacy compatibility program | derived-rebuildable | no runtime state; an independent production-source census is checked against ownership tags and matrix rows on every qualification run | indexed: fixed census entries plus bounded production-source symbols | n/a — source rows own archive/close behavior | not a defect — issue #2487 checker closes the former planned gap |
