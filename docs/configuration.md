...
| `merge_strategy` | `"merge" \| "rebase" \| "cherry-pick" \| "squash"` | `"merge"` | Branch merge strategy after Lean Turbo lane worktree completion. Lean retains commit-landing semantics by default; `"squash"` is available explicitly for worktree-only reviewable settlement. |
...
| `merge_strategy` | `"merge" \| "rebase" \| "cherry-pick" \| "squash"` | `"squash"` | Branch merge strategy after standard lane worktree completion. The default computes a synthetic result tree and applies only the bounded diff to the worktree, preserving the primary HEAD and index. |
