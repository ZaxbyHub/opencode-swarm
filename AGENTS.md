...
- Non-transient failures are ACTION-LOCAL and fail closed (issue #2103 semantic redefinition of the pre-2026-08 invocation-wide stop): parser, command-not-found, and sandbox-wrapper failures hard-stop immediately for the failing action's scope (shell execution); `general_permanent` and the taxonomy categories (`fs_*`, `git_*`, `abort_or_deadline`) hard-stop on the third consecutive same-category failure for the tool that recorded them. Immediate categories stay immutable across a late success — they clear only via the audited recovery transition (`recoverNonTransientCircuit`) or a verified new invocation/session. Non-immediate circuits clear when the same action succeeds. Read/diagnose/rescope/handoff/abort tools are never blocked by a circuit.
- Failures classify through the shared taxonomy (`src/utils/invocation-failure.ts`): provider patterns run only on dispatch-channel signals (never tool stdout), `command_not_found` requires structured proof (exit 127 / spawn ENOENT / shell-diagnostic grammar), ENOSPC/EPERM are `operator_action`, git conflicts are `repair_then_retry`, and containment denials are never retried into a bypass.
- A non-transient hard stop emits `telemetry.loopDetected` and one `NON-TRANSIENT STOP` advisory on the false-to-true transition. Agents must STOP retrying the failing action, report the blocker, and await corrected input, environment, or scope; recovery controls remain reachable while stopped.
