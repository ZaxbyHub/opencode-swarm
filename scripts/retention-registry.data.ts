...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	},
	{
		id: 'skill-usage',
		category: 1,
		pathGrammar: '.swarm/skill-usage.jsonl (+ .swarm/skill-usage.lock)',
		canonicalRoot: 'project-swarm',
		writerModules: ['src/hooks/skill-usage-log.ts'],
		writerCitations: [
			'src/hooks/skill-usage-log.ts:1187 appendSkillUsageEntry — locked append with torn-tail guard; O(1) manifest-header maintenance predicate (maintenanceDue); pressure gate stops operational-class writes (issue #2038)',
			'src/hooks/skill-usage-log.ts:1569 appendFeedbackAppliedMarker — locked marker append inside the feedback critical section (issue #2038)',
		],
		readerCitations: [
			'src/hooks/skill-usage-log.ts:1329 readSkillUsageEntries — bounded TAIL read, READ_MAX_BYTES=2MiB independent of file size (issue #2038)',
			'src/hooks/skill-usage-log.ts:1378 getSkillUsageCoverage — typed complete/truncated/empty disclosure (issue #2038)',
			'src/hooks/skill-usage-log.ts applySkillUsageFeedback — maintenance-path FULL read of entries+markers under the lock (steady state bounded by the retention envelope; #2037 honest-work model; a bounded window here would strand head-of-file unprocessed actionable entries — pressure could never clear) (issue #2038)',
			'src/hooks/skill-usage-log.ts:1408 readSkillUsageEntriesTail — TAIL 64 KiB (TAIL_BYTES_DEFAULT), unchanged (issue #2038)',
			'src/hooks/skill-scoring.ts formatSkillIndexWithContext — coverage-gated disclosure line; curator retire/revise/staleness skip on truncated coverage (issue #2038)',
		],
		schemaVersion: '1 (skill-usage-manifest header, issue #2038)',
		stateClass: 'derived-rebuildable',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'ACTIVE_MAX_BYTES=1MiB (manifest+entries+markers) / ACTIVE_MAX_ENTRIES=5k / AGE_MAX_MS=90d for OPERATIONAL entries; unprocessed compliant/violated entries are correctness-relevant and exempt (pressure path); PER-SKILL 500-entry selection is a policy INSIDE the ceilings',
			scope: 'global',
			citation: 'src/hooks/skill-usage-log.ts:165 SKILL_USAGE_LIMITS (issue #2038)',
		},
		readBound: { pattern: 'tail-bounded', bound: 'READ_MAX_BYTES=2MiB (records ≤ bytes/min-line; parse time O(bytes))', sync: true, citation: 'src/hooks/skill-usage-log.ts:514 readBoundedTail (issue #2038)' },
		lockModel: 'exclusive .swarm/skill-usage.lock (wx create, stale-broken 5 min) guards append, compaction, and the async feedback critical section (issue #2038)',
		crashBehavior: 'atomic single-file rewrite (PID-scoped tmp + rename with bounded Windows retry) — no partial-apply state; torn final tail re-framed before append; legacy header-less files migrated at first compaction (append-path migration postponed above 8 MiB to the phase-boundary pass) (issue #2038)',
		closePolicy: 'untouched — persists across sessions',
		resetPolicy: 'not reset',
		legacyCompatibility: 'normalizeComplianceVerdict maps legacy violation→violated; legacySkillUsageId for pre-id entries; pre-#2038 header-less JSONL reads unchanged and migrates at the first compaction (issue #2038)',
		healthSignal: 'skill_usage_health (counts-only; accepted/compacted/retained/dropped-age/corrupt/preserved-marker/pruned-marker-ids/pressure figures, issue #2038)',
		owner: '#2038',
		disposition: {
			kind: 'retain-by-design',
			issue: 2038,
			citation:
				'PR 10 shipped the bounded single-file store (src/hooks/skill-usage-log.ts SKILL_USAGE_LIMITS manifest header + retained window) with bounded deterministic readers (readBoundedTail, READ_MAX_BYTES=2MiB), the correctness-class marker lifecycle (unprocessed actionable entries survive; processed markers age out by reference liveness), a wx lock, literal stop-optional-writes pressure semantics, and the skill_usage_health signal; verified by tests/unit/hooks/skill-usage-global-bound.test.ts.',
			note: 'PR 10 shipped the hard global byte/age/count ceiling across all skills and marker types, bounded deterministic readers with coverage disclosure, cross-process-safe compaction, and the skill_usage_health signal.',
		},
	},
