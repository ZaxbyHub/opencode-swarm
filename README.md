...
- 🚦 **Gated GitHub Action publisher** — consumers can copy the default-off [two-job caller example](examples/github-action/swarm-publish-gated.yml), replace its intentionally non-runnable Action-ref sentinels with a reviewed immutable commit SHA, and use the [composite Action contract](action.yml) to process a trusted labeled issue or explicit dispatch. Prepare has read-only permissions and no publication token; publish requires a protected Environment with human reviewers, verifies the bound artifact, and atomically creates or reuses one pull request. See [the CI guide](docs/ci.md#gated-github-action-publisher-2498) for setup, pins, trust boundaries, idempotency, and failure behavior.
