...
Idempotent 4-stage terminal finalization. Before destructive cleanup or Git alignment, it prints an exact inventory and requires a one-shot `--confirm=<token>` from the preview; the inventory is revalidated after the finalize lock and scope drift aborts without mutation. Use `--dry-run` for the existing read-only stage preview.
