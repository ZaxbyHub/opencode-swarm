...
/**
 * The instant the retry/concurrency suites below pin `Date.now()` to.
 *
 * Those suites are about RETRY accounting, not the wall-clock budget, yet they
 * leave `deps.now` unset — so `drainSessionQueue` falls back to the live
 * `Date.now()` and evaluates `admissionConfig()`'s default 10 s
 * `max_drain_wall_time_ms` against real elapsed time. A slow or
 * coverage-instrumented run could therefore trip `budgetExhausted()` mid-suite
 * and turn a retry assertion into a deferral (issue #1782 root-cause class 1).
 * Freezing removes that coupling entirely; nothing here asserts on elapsed
 * time. The budget suites further down inject their own `now` and are
 * deliberately left alone.
 */
const FROZEN_NOW = Date.parse('2026-01-01T00:00:00.000Z');
...
describe('drainSessionQueue — concurrency bound', () => {
	it('never runs more admissions at once than max_concurrent_admissions', async () => {
		// Frozen for the same reason as the retry suite: the assertion is about
		// the in-flight PEAK, not elapsed time, and `Bun.sleep` is unaffected by
		// the `Date.now()` spy — so the real concurrency behavior is preserved
		// while the default 10 s drain budget can no longer interfere.
		await withFrozenClockAsync(
			async () => {
				for (let i = 0; i < 6; i++) {
					enqueueCandidate(
