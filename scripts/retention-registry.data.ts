...
export const RETENTION_REGISTRY: readonly RetentionRow[] = [
	...
	// ─────────────────────────────────────────────────────────────────────────
	{
		...
		writerCitations: [
			'src/background/pending-delegations.ts:2322 appendRecord — appendFileSync :2327 (19 mutation entry points :2474-4356)',
			'src/background/pending-delegations.ts:1069 writeDurableFileSync — fsync+rename-with-retry for checkpoint/manifest/rolled-tail (:2127-2168)',
		],
		readerCitations: [
			'src/background/pending-delegations.ts:2265 readDelegations — checkpoint+tail fold (lenient), sync',
			'src/background/pending-delegations.ts:2286 scanDelegationsForRecovery — strict, fails closed',
		...
		schemaVersion:
			'RecordSchema schemaVersion 1|2|3; checkpoint/manifest literal 1 (:963,:984,:994)',
		stateClass: 'authoritative',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'compaction high-water 1 MiB / low 256 KiB (:97-98); MAX_RECOVERY_LEDGER_BYTES 4 MiB (delegation-health.ts:34); MAX_CHECKPOINT_BYTES 2 MiB / 2048 records (:100,:104); TOMBSTONE_MIN_AGE 72 h (:111)',
			scope: 'global',
			citation: 'src/background/pending-delegations.ts:97-111; src/background/delegation-health.ts:34 (#2034)',
		},
		readBound: {
			...
			sync: true,
			citation: 'src/background/pending-delegations.ts:85-90,1492',
		},
		...
		disposition: {
			kind: 'not-a-defect',
			proof: 'Bounded by the #2034 checkpoint/tail compaction contract: 1 MiB high-water global trigger, 4 MiB hard recovery bound, 2 MiB/2048-record checkpoint validation, 72 h tombstone floor (src/background/pending-delegations.ts:97-111; src/background/delegation-health.ts:34).',
		},
	},
	{
	...
	},
	{
		...
		readerCitations: [
			'src/background/pending-delegations.ts:3809-3856 readDelegationFallback / listDelegationFallbacks / scanDelegationFallbacksForRecovery',
	...
	},
	{
		...
		category: 2,
		pathGrammar:
			'.swarm/pr-monitor/subscriptions.checkpoint.json (+ bounded subscriptions.audit.jsonl transition tail; transient legacy subscriptions.jsonl → subscriptions.legacy.jsonl archive, TTL-deleted; single quarantine slots subscriptions.checkpoint.foreign.json / .corrupt.json)',
		canonicalRoot: 'project-swarm',
		...
		writerCitations: [
			...
			'src/background/pr-subscriptions.ts flushAuditEvents/compactAuditTail — transition-only audit appends; high/low-water rewrite keeps newest ≤250 lines / ≤64 KiB',
		],
		readerCitations: [
		...
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'live-subscription cap: explicit maxSubscriptions (config default 20, max 100 — src/config/schema.ts) with store-side safety net default 20 when the caller omits it (PR_SUBSCRIPTION_LIMITS.defaultMaxActiveSubscriptions); terminal records compacted 60-high→30-newest + 30-day age into monotone terminalSummary counters; checkpoint pressure-guard 256 KiB (reporting-only — active records never dropped for bytes) with HARD read-side guards (512-record replay guard + 1 MiB file ceiling → invalid/quarantine/legacy-recovery, never a synchronous over-ceiling load); audit tail 500-line/128 KiB high-water → 250-line/64 KiB low-water rewrite; legacy migration folds at most legacySourceMaxBytes 64 MiB per store operation (explicit finite work budget — an over-ceiling legacy source is refused, never folded, never archived, and disclosed via health + the /swarm pr status footer with a repair hint, the pending-delegations MAX_RECOVERY_LEDGER_BYTES precedent) with 1 MiB crash-resumable progress chunks; the replay guards are WRITER-enforced too — terminal compaction runs before every persist and a folded live set that still exceeds the 512-record/1 MiB replay capacity refuses migration and fails writes with a loud capacity error while reads keep folding the legacy source exactly (never persisting a checkpoint the reader would reject, never archiving unabsorbed data); quarantine slots single-file bounded',
			scope: 'global',
		...
		crashBehavior: 'atomic tmp+rename checkpoint writes; migration progress persisted per 1 MiB chunk (resumable cursor); quarantine → checkpoint write → audit append ordering; crash-after-archive repaired idempotently; corrupt/foreign checkpoints quarantined to single slots and recovered from the legacy log',
	...
	},
	{
		...
		writerCitations: [
			'src/tools/repo-graph/indexed-storage.ts:524 syncIndexFromGraph — full-replace transaction: DELETE FROM edges/files/graph_meta (:551-553) then re-INSERT every node/edge (:563-605)',
		...
		readerCitations: [
			'src/tools/repo-graph/indexed-storage.ts:845 queryNodeByFile — indexed single-row SELECT by path/module_name (resolveTargetRow, :775-794), sync',
			'src/tools/repo-graph/indexed-storage.ts:893 loadSubgraphForFiles — bounded-neighbourhood closure via idx_edges_source/idx_edges_target (:796-818), sync',
		],
		schemaVersion: 'schema_migrations versioned (6, indexed-storage.ts:91-136)',
		stateClass: 'derived-rebuildable',
		privacyClass: 'metadata',
		writeLimits: {
			bound: 'full replace per save, not append-only (indexed-storage.ts:550-617 DELETE-then-reinsert); row count equals the live graph node/edge count, itself bounded by `repo_graph.max_files` (default 10,000, max 100,000 — src/config/schema.ts:1321)',
			scope: 'global',
			citation: 'src/tools/repo-graph/indexed-storage.ts:550-617; src/config/schema.ts:1321',
		},
		readBound: {
			pattern: 'indexed',
