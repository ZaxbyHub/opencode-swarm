...
- ABORT OR CANCEL IF UNRECOVERABLE: if the PR head cannot be fetched/checked out, the verification bind is unreachable, or bounded recovery is exhausted before arming, call \`abort_pr_workflow\` with \`mode: "PR_FEEDBACK"\`, \`kind: "recovery"\`, and a one-line \`reason\` instead of looping. Plain recovery/force aborts refuse while \`prFeedbackReadyToPublish\` is armed. To terminate an armed publication without publishing, call \`abort_pr_workflow\` with \`kind: "cancel-publication"\`, \`cancel_publication: true\`, and a non-empty \`reason\`; it records terminal \`cancelled_without_publication\` with the observed remote head, never grants push authority, and then clears the gate. Do not substitute a plain abort. To change approved content, call \`invalidate_pr_feedback_publication\` and rerun the full ladder. A published generation is never cleared by abort; use \`complete_pr_workflow\` so the actual remote branch is re-verified. Do NOT abort or cancel while useful bounded recovery work remains.
