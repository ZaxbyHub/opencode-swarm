...
// Regression pinning tests for the issue #2846 quarantine disposition.
//
// Issue #2846 was auto-filed by the merge-group flake-detection workflow
// (#1782, .github/workflows/flake-detection.yml) after CI run 35422590582
// (merge_group pr-2837, head bdeba4456f2cdf053368e2114a8f93f9f06cb173,
// 2026-09-19T04:55:50Z). It listed one candidate:
//   - tests/unit/commands/close-active-state-unlink-retry.test.ts
//     (windows-latest unit-shard 2, hard-failure EBUSY from rmSync in the
//     test's `finally` teardown on all three ci.yml retry attempts) ->
//     WINDOWS ledger (scripts/ci/quarantined-tests-windows.txt), because
//     the sibling ubuntu/macos shards were green and did not even include
//     this file in their expected-files lists (ext4/APFS/NTFS sort-order
//     differences put the close-* files in different shards per OS).
//
// Each pinning test below reads the real ledger files off disk and
// asserts the entry is present exactly once in the correct ledger and
// absent from the others, that its comment block carries the OWNER/EXPIRY
// metadata required by check:invariants Check 7 (issue #2477), and that
// the on-disk file exists at exactly the ledger path.
...
const ISSUE_2846_QUARANTINED_PATH =
	'tests/unit/commands/close-active-state-unlink-retry.test.ts';
...
describe('ci.yml integration — quarantine ledger entry for issue #2846 merge-group flake detection', () => {
	test('close-active-state-unlink-retry.test.ts is an active entry in the windows ledger', () => {
		// Regression guard for issue #2846: without the quarantine entry, the
		...
		expect(activeEntries(WINDOWS_LEDGER_PATH)).toContain(
			ISSUE_2846_QUARANTINED_PATH,
		);
	});

	test('the entry is scoped to the windows ledger only (single-OS evidence)', () => {
		// The #2846 flake was windows-latest-only with green ubuntu/macos
		...
		for (const ledger of [
			...
			expect(activeEntries(ledger)).not.toContain(ISSUE_2846_QUARANTINED_PATH);
		}
	});
	...
	test('the quarantined path exists and is discovered by the ci.yml find chain', () => {
		...
		expect(existsSync(join(REPO_ROOT, ISSUE_2846_QUARANTINED_PATH))).toBe(true);
	});
