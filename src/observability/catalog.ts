 ...
 *
 * Exactly 47 entries, matching the `TelemetryEvent` union at
...
export interface CatalogEntry {
	 ...
	 *
	 * `false` for all 47 entries today, and that is a truthful statement about
...
const CONSUMER_HEARTBEAT_LISTENER = Object.freeze(['src/telemetry.ts:186']);
...
const CATALOG_SOURCE: readonly (readonly [string, CatalogEntryInput])[] = [
	...
	// ── skill-usage storage (issue #2038) ─────────────────────────────────
	[
		'skill_usage_health',
		{
			// Emitted after a changing compaction / phase-boundary / feedback
			// pass (plus throttled pressure-drop and deferred-migration signals)
			// for the bounded `.swarm/skill-usage.jsonl` store (issue #2038).
			// Payload is accepted/compacted/retained/dropped/corrupt counts,
			// preserved/pruned marker-ID counts, pressure figures, and byte
			// totals — counts ONLY, no skill paths, no agent names, no file
			// content — so `operational` is the truthful privacy class. No live
			// consumer yet; PR 16/19 reporting will consume it.
			category: 'lifecycle',
			severity: 'notice',
			privacyClass: 'operational',
