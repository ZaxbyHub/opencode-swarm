...
describe('skill-usage bounds (issue #2038)', () => {
	...
	describe('poisoned-newestMs mass-eviction (PR #2347 review round 2)', () => {
		test('one far-future-timestamped entry does not evict every legitimately-recent entry', () => {
			...
			// `2026-08-2X` cohort is "recent" only while the suite happens to run
			// near that date; once wall-clock passes maxAgeMs (90d) past it, these
			// same fixed dates fall below the 90-day cutoff and the assertion
			// flips from 10 to 0 — a calendar time-bomb in the exact test meant to
			// guard this invariant. Deriving from `Date.now()` makes the fixture
			// permanently "recent" regardless of when the suite runs.
