...
describe('skill-usage bounds (issue #2038)', () => {
	...
	describe('poisoned-newestMs mass-eviction (PR #2347 review round 2)', () => {
		test('one far-future-timestamped entry does not evict every legitimately-recent entry', () => {
			...
			// timestamps are deterministic across runs, while still deriving
			// from Date.now() rather than a hardcoded literal: a hardcoded
			// "recent" cohort is recent only while the suite happens to run near
			// that date; once wall-clock passes maxAgeMs (90d) past it, fixed
			// dates fall below the 90-day cutoff and the assertion flips from
			// 10 to 0 — a calendar time-bomb in the exact test meant to guard
			// this invariant.
			withFrozenClock(
				() => {
					markMigrated(dir);
					...
					// far enough in the future to still be "the future" no matter
					// when this test runs.
					lines.push(
						rawEntry({
							id: 'poisoned-future',
					...
					// Un-clamped: cutoff = year-3000 minus 90 days is still year
					// 3000, so every real recent entry falls below it and is
					// mass-evicted in one pass — only the poisoned entry itself
					// would survive. Clamped: the cutoff anchors to
					// min(newestMs, Date.now()), so the ten legitimate entries
					// survive regardless of the poisoned one's timestamp.
					...
					// poisoned entry's own timestamp is always >= the clamped
					// cutoff, so it survives every pass too. Harmless (one record
					// out of maxEntries=5,000) and asserting it here means a future
					// change that decides to also age out poisoned-looking entries
					// makes that choice consciously, instead of this test silently
					// starting to fail.
