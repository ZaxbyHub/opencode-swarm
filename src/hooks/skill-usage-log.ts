...
function applyRetention(
	...
	if (newestMs !== Number.NEGATIVE_INFINITY) {
		...
		// timestamps then fall below the inflated cutoff. `Math.min` is a true
		// no-op for the entire legitimate case (newestMs is never ahead of
		// Date.now() when every write is wall-clock-stamped), so it does not
		// reintroduce the wall-clock dependence the anchor-to-data design
		// deliberately avoids for the repeated-cycle idempotency contract — it
		// only ever engages on the anomalous poisoned-timestamp path.
