...
Triggered by `/swarm pr-review`. A read-only, structured review: intent reconstruction -> all 6 fixed base explorer lanes launched with `dispatch_lanes_async` while the architect continues non-dependent PR inspection -> incremental `collect_lane_results` polling and final join barrier -> all 11 mandatory repository-agnostic micro-lanes, focused but never waived by diff/context signals -> independent reviewer confirmation -> critic challenge on HIGH/CRITICAL -> synthesis only after coverage closure. The architect checks out the PR branch locally before exploring (explorers read the working tree, not git history). Does NOT mutate source or delegate to the coder. Blocking dispatch and direct Task calls are not provenance-equivalent; if structured lanes cannot close coverage, the review is BLOCKED and surfaced to the user rather than degraded.
