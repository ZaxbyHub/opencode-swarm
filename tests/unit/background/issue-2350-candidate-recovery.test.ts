...
const baseRow = (evidence: string) =>
	`[CANDIDATE] | C-2350 | ${lane} | HIGH | correctness | src/a.ts:1 | claim | ${evidence} | impact | HIGH`;
...
describe('issue #2350 discovery recovery', () => {
	...
	test('recovers unescaped evidence pipes with explicit lossy classification', () => {
		...
		expect(prReviewDiscoveryArtifactCoversLane(text, lane)).toBe(true);
		expect(
			normalizeCandidateArtifact(normalized.text, 'base_explorer').repairKinds,
		).not.toContain('candidate-evidence-pipe-recovery-lossy');
	});

	test('recovers micro-lane evidence pipes without shifting the invariant field', () => {
		...
		expect(normalized.text).toContain(
			'trace alpha \\| trace beta \\| trace gamma',
		);
		expect(
	...
	test('does not guess an evidence boundary when the confidence suffix is invalid', () => {
		const text = `${CANDIDATE_HEADERS.base_explorer}\n${baseRow('trace a | trace b').replace(/HIGH$/, 'CERTAIN')}`;
		const normalized = normalizeCandidateArtifact(text, 'base_explorer');
		expect(normalized.repairKinds).not.toContain(
			'candidate-evidence-pipe-recovery-lossy',
		);
