...
- For a Profile-A review continuation, require the exact user command `/swarm
  pr-feedback <PR_URL> continue from
  .swarm/pr-review/<run_id>/feedback-handoff.json`; its mechanical transition
  validates terminal review state and creates an unbound feedback gate. The
  artifact by itself does not authorize writes.
