...
- User-supplied `working_directory` must resolve to a project root, never an ordinary subdirectory. A nested directory is an independent root only when it directly contains a `.git` file/directory (repository, linked worktree, or submodule declaration) or a `.opencode/` directory (#2127). These are explicit local declarations: direct malformed/empty `.git` markers still opt in, but marker symlinks/junctions and inaccessible markers do not. `save_plan`, the shared `resolveWorkingDirectory` helper, and every duplicate root guard must apply the same policy; ambiguous ancestor `.swarm` or project-indicator state fails closed. See also v6.82.2 (`#577`).
