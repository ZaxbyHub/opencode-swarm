 ...
 *
 * Budget: 750 ms per composed call — the issue's stated ceiling (hundreds-of-
 * ms class per assumption A-2), generous for CI variance, and far below the
 * seconds-class regressions being guarded. One warm-up invocation precedes the
 * measured calls so cold first-call dynamic imports cannot flake the guard
 * (fix plan "Round 2 revisions" item 6).
 *
 * Elapsed time is measured with the monotonic `performance.now()` rather than
 * `Date.now()` — the repo's test-clock gate (scripts/check-test-clock.ts)
 * blocks raw wall-clock usage in added test lines, and performance.now is the
 * established precedent for elapsed measurements (tests/unit/index.test.ts).
 */
import {
	afterAll,
	afterEach,
...
/** Per-composed-call ceiling (ms). See file header for the rationale. */
...
describe('composed tool.execute.after latency budget (issue #2472 W10 / AC-14)', () => {
	...
	test('the REAL composed tool.execute.after chain completes read-tool calls within the latency budget', async () => {
		...
		// (b) ONE warm-up invocation on the representative read call —
		// discarded timing; covers cold first-call dynamic imports.
		await toolAfter(makeInput('latency-warmup'), makeOutput());
